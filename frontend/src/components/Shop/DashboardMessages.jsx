...
const MessageList = ({
  ...
  const fileUrl =
    userData && userData.avatar && userData.avatar.url
      ? `${backend_url}${userData.avatar.url.replace(/^\/uploads\//, "")}`
      : "";
  ...
  return (
    ...
    >
      <div className="relative">
        <img src={fileUrl} alt="" className="w-[50px] h-[50px] rounded-full" />
        {online ? (
  ...
  return (
    <div className="w-full flex flex-col h-full">
          ...
          messages.map((item, index) => {
            return (
              <div
                ...
                ref={scrollRef}
                key={index} // Added key for unique identification of elements
              >
                ...
                {item.images &&
                  ...
                  item.images.map((image, imageIndex) => (
                    <img
                      src={`${backend_url}${image.replace(/^\/uploads\//, "")}`}
                      className="w-[300px] h-[300px] object-cover rounded-[10px] mr-2"
                      alt={`Message Image ${imageIndex}`}
                      key={imageIndex} // Added key for unique identification of elements
                    />
                  ))}
                {item.text !== "" && (
                  <div>
      ...
      >
        ...
        <div className="w-[97%] flex-1 flex">
          <input
            type="text"
            required
          ...
          <label htmlFor="send">
            <AiOutlineSend size={20} className="cursor-pointer ml-2" />
          </label>
        </div>
      </form>
    </div>
